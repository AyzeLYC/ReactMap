...
const getFormatters = ( locale ) => ( {
  ...
  timeFormat: new Intl.DateTimeFormat( locale, { hour: 'numeric', minute: 'numeric', second: 'numeric' } ),
  relativeFormat: new Intl.RelativeTimeFormat( locale, { numeric: 'always', style: 'short' } ),
  numberFormat: new Intl.NumberFormat( locale ),
  collator: new Intl.Collator( locale, { sensitivity: 'base', ignorePunctuation: true } ),
  /** @param { string } unit */
  distanceFormat: ( unit ) => new Intl.NumberFormat( locale, { unitDisplay: 'short', unit: unit.replace( /s$/, '' ), style: 'unit', maximumFractionDigits: 2 } )
  ...
  getRelative: ( epoch ) => {
    ...
    for ( let i = 0; i < UNITS.length; i++ ) {
      ...
      if ( count > 0 ) {
        ...
        result.push( rtf.format( isNegative ? -count : count, unit ).replace( '.', '' ) );
        seconds -= count * value;
    ...
    return result.filter( ( _, i ) => i < 2 ).map( ( r, i ) => {
