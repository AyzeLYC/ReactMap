  ...
  persist(
    ...
    (set, get) => ({
            ...
            filters: {
              ...
              scanAreas: {
                ...
                filter: {
                  ...
                  areas: [...existing].filter((area) => validAreas.includes(area)
      ...
      getPokemonFilterMode: () => {
        ...
        return filters?.pokemon?.easyMode && !userSettings?.pokemon?.legacyFilter ? 'basic' : userSettings?.pokemon?.legacyFilter ? 'expert' : 'intermediate'
    ...
    /** @type { ReturnType< import( '@rm/types' ).useDeepStore > [ 1 ] } */ (
      ...
      (...args) => {
        ...
        return useStorage.setState( (prev) => {
          ...
          const nextValue = args.length === 1 ? typeof args[ 0 ] === 'function' ? args[ 0 ]( prevValue ) : args[ 0 ] : args[ 1 ];
          ...
          return {
            [first] : first === path ? nextValue : setDeep(prev[first], path, nextValue)
          };
