 ...
 * @typedef {{
    ...
    feedback: boolean,
    drawer: boolean,
    advancedFilter: { open: boolean, category: import( '@rm/types' ).AdvCategories | '', id: string, selectedIds: string[] },
...
export const useLayoutStore = create( () => ( {
  ...
  drawer: false,
  slotSelection: '',
  advancedFilter: { open: false, category: '', id: '', selectedIds: [] },
  dialog: { open: false, category: '', type: '' },
  gymBadge: { open: false, gymId: '', badge: 0 }
...
export const toggleDialog = ( open, category, type, filter ) => () => {
  ...
  analytics( 'Menu Toggle', `Open: ${ open }`, `Category: ${ category } Menu: ${ type }` );
  
  useLayoutStore.setState( ( prev ) => ( {
    
    dialog: { open, category: category || prev.dialog.category, type: type || prev.dialog.type };
  ...
  if ( filter && type === 'filters' ) {
    ...
    useStorage.setState( ( prev ) => ( {
      ...
      filters: { ...prev.filters,  [ category ]: { ...prev.filters[ category ], filter } };
  ...
  if ( filter && type === 'options' ) {
    ...
    useStorage.setState( ( prev ) => ( { userSettings: { ...prev.userSettings, [ category ] : filter } }));
