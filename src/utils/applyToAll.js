...
export const STANDARD_BACKUP = /** @type {import('@rm/types/lib').BaseFilter} */ ( {
    enabled: false,
    size: 'md',
    all: false,
    adv: '',
 ...
 */
export function applyToAll( newFilter, category, selectedIds = [], includeSlots = false ) {
  ...
  const newObj = Object.fromEntries( staticFilters.flatMap( ([key, staticFilter]) => {
      ...
      const filters = [
        ...
        [
          key,
          idSet.has(key) ? { size: 'md', ...filter, ...newFilter, all: !!easyMode } : filter,
        ]
      ...
      if (key.startsWith('t') && +key.charAt(1) !== 0 && includeSlots) {
        ...
        filters.push(...Object.entries(generateSlots(key, newFilter.enabled, userFilters)));
